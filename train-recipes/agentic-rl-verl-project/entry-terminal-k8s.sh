#!/bin/bash
# Terminal fully-async veRL task using EKS-backed sandbox pods.
set -euo pipefail
...
TASK_CONFIG="${1:-${TASK_CONFIG:-$PROJ_DIR/aws/training/configs/terminal-k8s.yaml}}"
...
TRAIN_DATA="${TRAIN_DATA:-/fsx/data/terminal_verl/train.parquet}"
VAL_DATA="${VAL_DATA:-/fsx/data/terminal_verl/validation.parquet}"
NNODES="${INSTRT_NUM_NODES:-1}"
export TRAIN_GPUS="${TRAIN_GPUS:-4}" ROLLOUT_GPUS="${ROLLOUT_GPUS:-4}"
RUN="terminal-k8s-$(date +%Y%m%d_%H%M%S)"
PROJECT_NAME="${PROJECT_NAME:-agentic-rl-terminal-k8s}"
PROMPT_LEN="${PROMPT_LEN:-2048}"
RESPONSE_LEN="${RESPONSE_LEN:-16384}"
MAX_ASSISTANT_TURNS="${MAX_ASSISTANT_TURNS:-25}"
MAX_TOOL_RESPONSE_LENGTH="${MAX_TOOL_RESPONSE_LENGTH:-4000}"
TOOL_CONFIG="${TOOL_CONFIG:-$PROJ_DIR/tooluse/configs/terminal_k8s_tool_config.yaml}"
REWARD_FUNCTION_PATH="${REWARD_FUNCTION_PATH:-$PROJ_DIR/tooluse/rewards/terminal_reward.py}"
...
if [ ! -f "$TRAIN_DATA" ]; then
    echo "ERROR: Terminal veRL data not found at $TRAIN_DATA"
    echo "Prepare it first with the original project's terminal data pipeline, then sync to FSx."
    exit 1
fi
